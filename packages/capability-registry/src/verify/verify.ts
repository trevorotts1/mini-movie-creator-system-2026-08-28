/**
 * CAP-009 — Provider health/verify command (Model Capability Registry).
 *
 * Runbook §61 contract for `mmcs providers verify`:
 *   reports configured capability; documented capability; runtime-observed
 *   capability where safely testable; last verified date; discrepancy
 *   warning. Never silently rewrite VERIFIED capabilities based on one
 *   transient failure.
 *
 * Three capability "views" per provider/model:
 *   - CONFIGURED: what the operator's local config actually wires up right
 *     now (env keys present, models selected) — supplied by the caller.
 *   - DOCUMENTED: what the capability registry says the model can do
 *     (the CAP-001 MediaModelCapability profile + its confidence tier and
 *     lastVerifiedAt) — supplied by the caller.
 *   - OBSERVED: what a safe runtime probe just saw. The probe is injected
 *     (async function), never built here — this module performs no I/O.
 *
 * The transient-failure rule is the load-bearing one: when a probe throws,
 * times out, or returns an error-shaped result, that single observation is
 * recorded as `TRANSIENT` and can NEVER downgrade or rewrite an existing
 * VERIFIED confidence. Only an observed result that materially differs from
 * the documented capability produces a DISCREPANT entry (a warning), and
 * applying observed overrides back into the registry is an explicit,
 * deliberate act (`applyOverrides` returns a NEW registry object — it never
 * mutates the input, and it never promotes a TRANSIENT observation).
 *
...
export function applyOverrides(
  ...
  for (const override of overrides) {
    ...
    const nextVerified = entry.verifiedFacts
      ? { ...entry.verifiedFacts }
      : undefined;
    const current = nextFacts[override.field];
