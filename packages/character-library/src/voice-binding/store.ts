/**
 * Voice-profile persistence — JSON-file backed store, one profile per
 * character (character-library side of the Fish voice binding).
 *
 ...
 * is practical later). All writes serialize; concurrent calls from one process
 * are safe. One profile per character ID — the binding is permanent (spec
 * §30: recurring characters never randomly change voices); creating a second
 * profile for the same character is an error, updates are the path.
 */
...
export type VoiceProfileMap = Record<string, VoiceProfile>;

/** The on-disk document shape. Versioned so the format can evolve. */
export interface VoiceProfileFile {
  formatVersion: 1;
  profiles: VoiceProfileMap;
}

/** Injectable filesystem seam for tests. Mirrors the subset of `fs.promises`
 * the store uses. */
export interface VoiceProfileFs {
  readFile(path: string, encoding: "utf8"): Promise<string>;
  writeFile(path: string, data: string, encoding: "utf8"): Promise<void>;
  mkdir(dir: string, options: { recursive: true }): Promise<string | undefined>;
}

const DEFAULT_FS: VoiceProfileFs = fs;
...
export interface VoiceProfileStoreOptions {
  ...
  fs?: VoiceProfileFs;
  /** Injectable clock for createdAt/updatedAt stamps (tests). */
  now?: () => Date;
}
...
export class VoiceProfileStore {
  ...
  private readonly fsImpl: VoiceProfileFs;
  ...
  constructor(options: VoiceProfileStoreOptions) {
    if (!options.filePath?.trim()) {
      throw new Error("VoiceProfileStore.filePath is required");
    }
   ...
   * recurring characters never randomly change voices). */
  async create(
    characterId: string,
  ...
  async get(characterId: string): Promise<VoiceProfile | undefined> {
    const doc = await this.readDoc();
  ...
  /** List profiles bound to any of the given character IDs (deduped — a
   * repeated ID yields its profile once, first-occurrence order). */
  async listForCharacters(
  ...
  async update(
    ...
    return this.enqueue(async () => {
      ...
      const updated = withClock(
        updateVoiceProfile(existing, patch),
        this.now,
        existing,
      );
  ...
  /** Record a test-sample result on the profile (spec §30: test sample
   * status). */
  ...
  /** True when the profile for `characterId` exists, is approved, and its
   * test sample is approved — the gate before auto-reuse in synthesis
   * planning. */
  ...
  private async readDoc(): Promise<VoiceProfileFile> {
    try {
      const raw = await this.fsImpl.readFile(this.filePath, "utf8");
      const parsed = JSON.parse(raw) as Partial<VoiceProfileFile>;
      if (
...
/** Stamp createdAt/updatedAt with the store's injected clock (tests) while
...
function withClock(
  ...
  if (previous) {
    return { ...profile, updatedAt: stamp };
  }
