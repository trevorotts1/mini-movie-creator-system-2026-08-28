...
/**
 * Bounded exponential backoff (MMCS task GHL-011).
 *
 ...
 * any single wait, so a flaky GHL endpoint can never wedge archival.
 *
...
export const DEFAULT_BACKOFF: Required<Pick<BackoffOptions, "baseDelayMs" | "maxDelayMs" | "maxAttempts">> = {
  baseDelayMs: 250,
  maxDelayMs: 8_000,
  maxAttempts: 4,
};
...
export function resolveBackoff(options: BackoffOptions = {}): Required<BackoffOptions> {
  ...
  const maxAttempts = Math.max(1, Math.floor(finitePositive(options.maxAttempts, DEFAULT_BACKOFF.maxAttempts)));
  return {
    baseDelayMs,
    maxDelayMs,
    maxAttempts,
