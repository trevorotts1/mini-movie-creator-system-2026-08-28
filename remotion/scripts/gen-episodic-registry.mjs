...
// SKR-009: falling back to the example plan is allowed (it keeps `npm run gen` and
// `tsc --noEmit` green on a fresh checkout) but it must never be SILENT. A registry built
// from placeholder data produces compositions whose only content is a debug overlay, and a
// render of one looks like a success while being scaffolding. The generated registry now
// carries `placeholder: true`, which the composition uses to show its debug overlay — so
// the difference between "real episode" and "example data" is recorded in the artefact
// rather than inferred by whoever reads the output.
const usingExample = !existsSync(planPath);
const planFile = usingExample ? examplePath : planPath;
...
for (const episode of episodes) {
  ...
  compositions.push({
    compositionId,
    // Marks the whole registry as placeholder-derived; see `usingExample` above.
    ...(usingExample ? { placeholder: true } : {}),
    episodeCode: code,
    seriesId: series.id,
...
if (usingExample) {
  console.warn(
    'gen-episodic-registry: WARNING — no episodic-plan.json found; generated from ' +
      'episodic-plan.example.json. These compositions are PLACEHOLDER data and render a ' +
      'debug overlay instead of episode content. Do not treat a successful render of one ' +
      'as an episode.',
  );
}
